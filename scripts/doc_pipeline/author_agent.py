...
_PLUGIN_SYSTEM_PROMPTS: dict[str, str] = {
    "/write-spec": (
        ...
        "Positioning, Business Impact. Be specific, not generic. Cite evidence "
        "where the brief points to it. Output only the spec, no preamble."
    ),
    "/roadmap-update": (
    ...
    "/metrics-review": (
        ...
        "metrics in favor of measurable, time-bound, actionable ones."
    ),
    "/synthesize-research": (
...
def author_draft(
    ...
    if mode == "plugin":
        ...
        system_prompt = (
            ...
            "Be specific, not generic. Cite evidence with [file:path] anchors "
            "where the brief points to source files. Numbers and durations are "
            "preferred over vague language. Output ONLY the doc body in valid "
            "markdown. Do not include preamble. Do not use em dashes (use commas, "
            "colons, periods, or parentheses instead). Do not use these phrases: "
            "synergize, leverage AI, best-in-class, world-class, cutting-edge, "
            "revolutionize, seamlessly, robust solution, game-changer, paradigm "
            "shift, ecosystem, democratize, disrupt, holistic, unlock value.\n\n"
